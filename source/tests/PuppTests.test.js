...
describe('basic navigation for BJ', () => {
    beforeAll(async () => {
        await page.goto('http://127.0.0.1:8087/Login/Login.html');
    ...
    */
    ...
    it('Test2: create an account and login - shows index page ', async () => {
        ...
        const url = await page.evaluate(() => location.href);
        expect(url).toMatch('http://127.0.0.1:8087/Index/Index.html');
    });

    it('Test3: From index page go back, should be login page ', async () => {
        await page.goBack();
        const url = await page.evaluate(() => location.href);
        expect(url).toMatch('http://127.0.0.1:8087/Login/Login.html');
    });
    ...
    it('Test6: go to index screen, make sure highlighted day is the current day', async () => {
        await page.goto('http://127.0.0.1:8087/Index/Index.html');
        await page.waitForTimeout(300);
    ...
    it('Test47: check that the home button works in the yearly overview', async () => {
        const indexURL = 'http://127.0.0.1:8087/Index/Index.html';
        await page.goto(
            'http://127.0.0.1:8087/YearlyOverview/YearlyOverview.html#2021'
        );
    ...
    it('Test48: check that the home button works in the monthly overview', async () => {
        const indexURL = 'http://127.0.0.1:8087/Index/Index.html';
        await page.goto(
            'http://127.0.0.1:8087/MonthlyOverview/MonthlyOverview.html#06/2021'
        );
        await page.waitForTimeout('300');
        await page.$eval('#house > a', (btn) => {
            btn.click();
    ...
    it('Test49: check that the home button works in the daily overview', async () => {
        const indexURL = 'http://127.0.0.1:8087/Index/Index.html';
        await page.goto(
            'http://127.0.0.1:8087/DailyOverview/DailyOverview.html#06/08/2021'
        );
        await page.waitForTimeout('300');
